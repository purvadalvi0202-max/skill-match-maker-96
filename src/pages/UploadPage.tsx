      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Button onClick={handleAnalyze} className="flex-1 h-12 rounded-xl gradient-primary text-primary-foreground border-0 text-base" disabled={!selectedJob || files.length === 0 || analyzing}>
              🔍 Analyze {files.length} Resume{files.length !== 1 ? 's' : ''}
            </Button>
            <Button variant="outline" onClick={handleDemoAnalyze} className="flex-1 h-12 rounded-xl text-base" disabled={!selectedJob || analyzing}>
              🎯 Run Demo Analysis
            </Button>
          </div>
        </motion.div>
